jQuery(function($) {
     ...
     */
    $(document).on('click', '.cf_icon_modal', function(e) {
        // Keep category item click from being executed.
        e.stopPropagation();
        // Prevent navigating to '#'.
        e.preventDefault();
    ...
    $($('[name="cf_save_icon"]')).on('submit', function(e) {
        e.stopPropagation();
        e.preventDefault();
        var form_data = $(this).serialize();
        $('#saveEdit_CF_Icon').addClass('bookme-progress');
        $.ajax({
            type: "POST",
            url: ajaxurl+'?action=CustomField_Icon_Save',
            data: form_data,
            success: function(response){
                if(response != 0) {
                    $('#modal_cf_icon_url').modal('hide');

                    quickadAlert({success: ['Successfully edited.']});
                }else{
                    quickadAlert({error: ['Problem in saving, Please try again.']});
                }
                $('#saveEdit_CF_Icon').removeClass('bookme-progress');
            }
        });
        return false;
    });
