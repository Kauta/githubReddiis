...
function checkSocialUser($userData = array(),$picname){
    ...
    if(!empty($userData)){
        ...
        if(!empty($info)){
            ...
            $insert_user->oauth_provider = escape_html($userData['oauth_provider']);
            $insert_user->oauth_uid = escape_html($userData['oauth_uid']);
            $insert_user->status = '1';
            $insert_user->name = escape_html($fullname);
            $insert_user->username = escape_html($username);
            $insert_user->password_hash = $pass_hash;
            $insert_user->email = escape_html($userData['email']);
            $insert_user->sex = escape_html($gender);
            $insert_user->image = escape_html($picname);
            $insert_user->oauth_link = escape_html($userData['link']);
            $insert_user->created_at = $now;
            $insert_user->updated_at = $now;
            $insert_user->country = escape_html($location['country']);
            $insert_user->city = escape_html($location['city']);
            $insert_user->save();
