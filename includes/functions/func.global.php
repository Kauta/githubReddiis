...
function verify_envato_purchase_code($code) {
    ...
    $url = "https://bylancer.com/api/api.php?verify-purchase=" . $code . "&version=" . $installing_version . "&site_url=" . $site_url . "&email=" . $buyer_email."&author=bylancer-tech";
    // Open cURL channel
    $ch = curl_init();
...
function escape_html($input){
    ...
    if(is_array($input)){
        ...
        foreach ($input as $key => $value) {
            $pureArray[$key] = $purifier->purify($value);
        }
        return $pureArray;
    }else{
...
function add_firebase_notification($SenderName,$SenderId,$OwnerName,$OwnerId,$productId,$productTitle,$type,$message)
{
    ...
    if($OwnerId){
        ...
        $insert_note->sender_name = escape_html($SenderName);
        $insert_note->sender_id = escape_html($SenderId);
        $insert_note->owner_name = escape_html($OwnerName);
        $insert_note->owner_id = escape_html($OwnerId);
        $insert_note->product_id = escape_html($productId);
        $insert_note->product_title = escape_html($productTitle);
        $insert_note->type = escape_html($type);
        $insert_note->message = escape_html($message);
        $insert_note->save();
