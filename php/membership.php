...
if(checkloggedin())
{
    if(isset($_POST['upgrade']))
    {
        $info = ORM::for_table($config['db']['pre'].'subscriptions')
            ->where('sub_id', escape_html($_POST['upgrade']))
            ->find_one();
        ...
        if(isset($_POST['payment_method_id']))
        {
            ...
            $info = ORM::for_table($config['db']['pre'].'payments')
                ->where(array(
                    'payment_id' => escape_html($_POST['payment_method_id']),
                    'payment_install' => '1'
                ))
                ->find_one();
            ...
            if($folder == "2checkout"){
                $_SESSION['quickad'][$access_token]['firstname'] = escape_html($_POST['checkoutCardFirstName']);
                $_SESSION['quickad'][$access_token]['lastname'] = escape_html($_POST['checkoutCardLastName']);
                $_SESSION['quickad'][$access_token]['BillingAddress'] = escape_html($_POST['checkoutBillingAddress']);
                $_SESSION['quickad'][$access_token]['BillingCity'] = escape_html($_POST['checkoutBillingCity']);
                $_SESSION['quickad'][$access_token]['BillingState'] = escape_html($_POST['checkoutBillingState']);
                $_SESSION['quickad'][$access_token]['BillingZipcode'] = escape_html($_POST['checkoutBillingZipcode']);
                $_SESSION['quickad'][$access_token]['BillingCountry'] = escape_html($_POST['checkoutBillingCountry']);
            }
        ...
        else
        {
            ...
            $page->SetParameter ('UPGRADE', escape_html($_POST['upgrade']));
            $page->SetParameter ('PAYMENT_METHOD_COUNT', $num_rows);
            $page->SetParameter ('SUB_ID', escape_html($_POST['upgrade']));
	...
	elseif(check_user_upgrades($_SESSION['user']['id']))
	{
		...
		if(isset($_GET['change_plan']) &&escape_html( $_GET['change_plan']) == "changeplan")
		{
