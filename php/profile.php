...
$limit = isset($_GET['limit']) ? escape_html($_GET['limit']) : 6;
$sorting = isset($_GET['sort']) ? escape_html($_GET['sort']) : "Newest";

if(isset($_GET['username'])){

    $keywords = isset($_GET['keywords']) ? str_replace("-"," ",escape_html($_GET['keywords'])) : "";
    ...
    if(isset($_GET['subcat']) && !empty($_GET['subcat'])){
        ...
        if(is_numeric($_GET['subcat'])){
            if(check_sub_category_exists($_GET['subcat'])){
                $subcat = escape_html($_GET['subcat']);
            }
        }else{
            $subcat = get_subcategory_id_by_slug(escape_html($_GET['subcat']));
        }
    }elseif(isset($_GET['cat']) && !empty($_GET['cat'])){
        if(is_numeric($_GET['cat'])){
            if(check_category_exists($_GET['cat'])){
                $category = escape_html($_GET['cat']);
            }
        }else{
            $category = get_category_id_by_slug(escape_html($_GET['cat']));
        }
    }
  ...
  END),";
    ...
    {
        $where.= "AND (city = '".escape_html($_GET['city'])."') ";
    }
    elseif(isset($_GET['location']) && !empty($_GET['location']))
    {
        $placetype = escape_html($_GET['placetype']);
        $placeid = escape_html($_GET['placeid']);
    ...
    $get_userdata = get_user_data(escape_html($_GET['username']));
...
FROM `".$config['db']['pre']."product`
 WHERE status = 'active' AND hide = '0' AND user_id = $user_id ";
        ...
        $page->SetParameter ('OVERALL_HEADER', create_header(escape_html($_GET['username'])." ".$lang['PROFILE']));
        ...
        $page->SetParameter ('USERNAME', escape_html($_GET['username']));
