...
// Check if this is an Username availability check from signup page using ajax
    if(isset($_POST["username"])) {
        ...
        if(preg_match('/[^A-Za-z0-9]/',escape_html($_POST['username'])))
        {
            $username_error = $lang['USERALPHA'];
            echo "<span class='status-not-available'> ".$username_error." [A-Z,a-z,0-9]</span>";
            exit;
        }
        elseif( (strlen(escape_html($_POST['username'])) < 4) OR (strlen(escape_html($_POST['username'])) > 16) )
        {
        ...
        else
        {
            if(checkloggedin())
            {
                ...
                {
                    $user_count = check_username_exists(escape_html($_POST["username"]));
                    if($user_count>0) {
...
// Check if this is an Email availability check from signup page using ajax
    if(isset($_POST["email"])) {
        ...
        elseif(!preg_match($regex, escape_html($_POST['email'])))
        {
        ...
        if(checkloggedin())
        {
            ...
            {
                $user_count = check_account_exists(escape_html($_POST["email"]));
                if($user_count>0) {
...
function removeImage(){
    ...
    if(isset($_POST['product_id'])){
        $id = escape_html($_POST['product_id']);
        $info = ORM::for_table($config['db']['pre'].'product')->select('screen_shot')->find_one(escape_html($_POST['product_id']));

        $screnshots = explode(',',$info['screen_shot']);
        if($key = array_search(escape_html($_POST['imagename']),$screnshots) != -1){
...
function email_contact_seller(){
    ...
    if (isset($_POST['sendemail'])) {
        ...
        $page->SetParameter('SENDER_NAME', escape_html($_POST['name']));
        $page->SetParameter('SENDER_EMAIL', escape_html($_POST['email']));
        $page->SetParameter('SENDER_PHONE', escape_html($_POST['phone']));
        ...
        $page->SetParameter('SENDER_EMAIL', escape_html($_POST['email']));
        $page->SetParameter('SENDER_PHONE', escape_html($_POST['phone']));
        $page->SetParameter('MESSAGE', escape_html($_POST['message']));
...
{
    global $config;
    $country_code = isset($_POST['id']) ? escape_html($_POST['id']) : 0;
    $selectid = isset($_POST['selectid']) ? escape_html($_POST['selectid']) : "";
...
{
    global $config;
    $state_id = isset($_POST['id']) ? escape_html($_POST['id']) : 0;
    $selectid = isset($_POST['selectid']) ? escape_html($_POST['selectid']) : "";
...
{
    global $config;
    $country_code = isset($_POST['country']) ? escape_html($_POST['country']) : "";
    $state = isset($_POST['state']) ? escape_html($_POST['state']) : "";
    $city_name = isset($_POST['city']) ? escape_html($_POST['city']) : "";
...
{
    global $config;
    $dataString = isset($_GET['q']) ? escape_html($_GET['q']) : "";
    $sortname = check_user_country();

    $perPage = 10;
    $page = isset($_GET['page']) ? escape_html($_GET['page']) : "1";
...
INNER JOIN `".$config['db']['pre']."subadmin1` AS s ON s.code = c.subadmin1_code and s.active = '1'
 ...
 ORDER BY
  ...
  END ";
    ...
    $pages  = ceil(escape_html($_GET["rowcount"])/$perPage);
...
{
    global $config,$lang;
    $dataString = isset($_POST['dataString']) ? escape_html($_POST['dataString']) : "";
    $sortname = check_user_country();
...
function setFavAd()
{
    ...
    $num_rows = ORM::for_table($config['db']['pre'].'favads')
        ->where(array(
            'user_id' => escape_html($_POST['userId']),
            'product_id' => escape_html($_POST['id'])
        ))
        ->count();

    if ($num_rows == 0) {
        $insert_favads = ORM::for_table($config['db']['pre'].'favads')->create();
        $insert_favads->user_id = escape_html($_POST['userId']);
        $insert_favads->product_id = escape_html($_POST['id']);
        $insert_favads->save();
...
{
    global $config;
    $id = isset($_POST['catid']) ? escape_html($_POST['catid']) : 0;
    $selectid = isset($_POST['selectid']) ? escape_html($_POST['selectid']) : "";
...
{
    global $config,$lang;
    $maincatid = isset($_POST['catid']) ? escape_html($_POST['catid']) : 0;
    $subcatid = isset($_POST['subcatid']) ? escape_html($_POST['subcatid']) : 0;
...
function getlocHomemap()
{
    ...
    if(isset($_GET['serachStr'])){
        $serachStr = escape_html($_GET['serachStr']);
    }
    else{
        $serachStr = '';
    }
    ...
    if(isset($_GET['state'])){
        $state = escape_html($_GET['state']);
    }
    else{
        $state = '';
    }
    if(!empty($_GET['city'])){
        $city = escape_html($_GET['city']);
    }
    else{
        if(!empty($_GET['locality'])){
            $city = escape_html($_GET['locality']);
        }else{
            $city = '';
        }
    }
    if(isset($_GET['searchBox'])){
        $searchBox = escape_html($_GET['searchBox']);
    }
    else{
        $searchBox = '';
    }
...
{
    global $config;
    $result = ORM::for_table($config['db']['pre'].'product')->find_one(escape_html($_POST['id']));
...
function quickad_ajax_home_search()
{
    ...
    $qString      = '';
    $qString      = escape_html($_POST['tagID']);
 ...
 ORDER BY
  ...
  END ";
        ...
        foreach ($rows as $info) {
            ...
            $subcatTerms[$info['sub_cat_id']]['picture'] = $info['picture'];
            if($config['lang_code'] != 'en' && $config['userlangsel'] == '1'){
...
function submitBlogComment(){
    ...
    if (!($is_login || isset($_SESSION['admin']['id']))) {
        if (empty($_POST['user_name']) || empty($_POST['user_email'])) {
            ...
            $email = escape_html($_POST['user_email']);
        ...
        if (!empty($_POST['commenting-as'])) {
            if (in_array($_POST['commenting-as'], array('admin', 'user'))) {
                $commenting_as = escape_html($_POST['commenting-as']);
            }
        }
    ...
    if (empty($_POST['comment'])) {
        ...
        $comment = validate_input(escape_html($_POST['comment']));
    }

    $duplicates = ORM::for_table($config['db']['pre'] . 'blog_comment')
        ->where('blog_id', escape_html($_POST['comment_post_ID']))
        ->where('name', $name)
        ->where('email', $email)
    ...
    if (!$comment_error) {
        ...
        $blog_cmnt->blog_id = escape_html($_POST['comment_post_ID']);
        ...
        $blog_cmnt->parent = escape_html($_POST['comment_parent']);
        $blog_cmnt->save();
