...
if(isset($_GET['archive'])){
    if(escape_html($_GET['archive']) == 'category' && !empty($_GET['keyword'])){
        $archive_cat = ORM::for_table($config['db']['pre'] . 'blog_categories')
            ->where('slug', escape_html($_GET['keyword']))
            ->find_one();
        ...
        $archive_author = ORM::for_table($config['db']['pre'] . 'admins')
            ->where('username', escape_html($_GET['keyword']))
            ->find_one();
...
if($category){
    ...
    $page_link = $link['BLOG-CAT'].'/'.escape_html($_GET['keyword']);
}
...
if($author){
    ...
    $page_link = $link['BLOG-AUTHOR'].'/'.escape_html($_GET['keyword']);
}

if(isset($_GET['s'])){
    $search = str_replace("-"," ",escape_html($_GET['s']));
