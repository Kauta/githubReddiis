...
{
    $confirm = escape_html($_GET["confirm"]);
    $get_user_id = escape_html($_GET["user"]);
    $check_confirm = 0;
    ...
    if($check_confirm)
    {
        ...
        $user_info = get_user_data(null,escape_html($_GET['user']));
...
if(isset($_POST["submit"])) {
    ...
    $password_error = '';
    $recaptcha_error = '';
    $name_length = strlen(utf8_decode(escape_html($_POST['name'])));
    if(empty($_POST["name"])) {
        $errors++;
    ...
    elseif(preg_match('/[^A-Za-z0-9]/',escape_html($_POST['username'])))
    {
        $errors++;
    ...
    $_POST["email"] = strtolower(escape_html($_POST["email"]));
    ...
    if($errors == 0) {
        ...
        $loggedin = userlogin($username, $_POST['password']);
...
if(isset($_POST['submit']))
{
    $page->SetParameter ('NAME_FIELD', escape_html($_POST['name']));
    $page->SetParameter ('USERNAME_FIELD', escape_html($_POST['username']));
    $page->SetParameter ('EMAIL_FIELD', escape_html($_POST['email']));
