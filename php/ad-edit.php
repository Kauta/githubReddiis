        ...
        if (!count($errors) > 0) {
            ...
            if (checkloggedin()) {
                ...
                if($config['post_desc_editor'] == 1)
                    $description = addslashes(escape_html($_POST['content']));
                else
                ...
                $info = ORM::for_table($config['db']['pre'].'product')
                    ...
                    ->find_one(escape_html($_POST['product_id']));
                ...
                if($item_status == "pending" or $config['post_auto_approve'] == 1)
                {
                    ...
                    $item_edit->set('product_name', escape_html($post_title));
                    $item_edit->set('slug', escape_html($slug));
                    $item_edit->set('category', escape_html($_POST['catid']));
                    $item_edit->set('sub_category', escape_html($_POST['subcatid']));
                    $item_edit->set('description', escape_html($description));
                    $item_edit->set('price', escape_html($price));
                    $item_edit->set('negotiable', $negotiable);
                    $item_edit->set('phone', escape_html($phone));
                    $item_edit->set('hide_phone', $hide_phone);
                    $item_edit->set('location', escape_html($location));
                    $item_edit->set('city', escape_html($cityid));
                    $item_edit->set('state', escape_html($state));
                    $item_edit->set('country', escape_html($country));
                    $item_edit->set('latlong', $latlong);
                    $item_edit->set('screen_shot', escape_html($item_screen));
                    $item_edit->set('tag', escape_html($tags));
                    $item_edit->set('videourl', escape_html($videourl));
                ...
                elseif($item_status == "active" or $item_status == "softreject" or $item_status == "expire")
                {
                    ...
                    $item_insrt->product_id = escape_html($_POST['product_id']);
                    ...
                    $item_insrt->description = escape_html($description);
                    $item_insrt->price = escape_html($price);
                    $item_insrt->negotiable = $negotiable;
                    $item_insrt->phone = escape_html($phone);
                    $item_insrt->hide_phone = $hide_phone;
                    $item_insrt->location = escape_html($location);
                    $item_insrt->city = escape_html($_POST['city']);
                    $item_insrt->state = escape_html($state);
                    $item_insrt->country = escape_html($country);
                    $item_insrt->latlong = escape_html($latlong);
                    $item_insrt->screen_shot = escape_html($item_screen);
                    $item_insrt->tag = escape_html($tags);
                    $item_insrt->videourl = escape_html($videourl);
                    $item_insrt->created_at = $timenow;
                    $item_insrt->comments = escape_html($_POST['comments']);
                    $item_insrt->save();
                }

                $product_id = escape_html($_POST['product_id']);
...
if(checkloggedin()) {
    ...
    if(check_valid_author($_GET['id'])){
        ...
        global $errors, $custom_fields, $catid,$catName, $subcatid,$subcatName, $title, $description, $price, $negotiable, $phone, $hide_phone, $tags, $videourl, $cityid, $mapLat, $mapLong, $seller_name, $seller_email;
        ...
        if (isset($info)  > 0) {
            ...
            $page->SetParameter ('VIDEOURL', $videourl);
