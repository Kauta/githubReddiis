...
if(checkloggedin())
{
    ...
    {
        $post_username = escape_html($_POST["username"]);
        // Check if this is an Username availability check from signup page using ajax
        if($post_username != $_SESSION['user']['username'])
        {
            ...
            elseif( (strlen($_POST['username']) < 4) OR (strlen($_POST['username']) > 16) )
            {
                ...
                $user_count = check_username_exists($post_username);
                if($user_count>0) {
                    $errors++;
        ...
        elseif($post_email != $ses_userdata['email'])
        {
            ...
            if (!preg_match($regex, $_POST['email'])) {
                ...
                $user_count = check_account_exists($post_email);
                if ($user_count > 0) {
                    $errors++;
        ...
        if($errors == 0)
        {
            ...
            if($oldemail != $post_email){
                $person->set('status', '0');
            }
            $person->set('username', $post_username);
            $person->set('email', $post_email);
