...
if ($num_rows > 0) {
    ...
    $item_id = $info['id'];
    $item_title = escape_html($info['product_name']);
    $item_videourl = escape_html($info['videourl']);
    $item_status = $info['status'];
    $item_featured = $info['featured'];
    $item_urgent = $info['urgent'];
    $item_highlight = $info['highlight'];
    $item_description = stripslashes(nl2br(escape_html($info['description'])));

    $showmore = (strlen($item_description) > 1000)? 1 : 0;
    $item_tag = escape_html($info['tag']);
    $item_location = escape_html($info['location']);
    ...
    $item_catid = escape_html($info['category']);
    ...
    $item_title_slug = create_slug(escape_html($item_title));
    ...
    $item_phone = escape_html($info['phone']);
    ...
    $price = price_format(escape_html($info['price']),$info['country']);
    $item_price = $price;
    ...
    if($info['videourl'] != ""){
        $item_videourl = str_replace("https://www.youtube.com/watch?v=","https://www.youtube.com/embed/",$info['videourl']);
        $show_videourl = 1;
    }else{
        $item_videourl = "";
        $show_videourl = 0;
...
$meta_desc = substr(strip_tags(escape_html($item_description)),0,150);
$meta_desc = trim(preg_replace('/\s\s+/', ' ', escape_html($meta_desc)));
// Output to template
...
$page->SetParameter ('ITEM_VIDEOURL', $item_videourl);
$page->SetParameter ('SHOW_VIDEOURL', $show_videourl);
