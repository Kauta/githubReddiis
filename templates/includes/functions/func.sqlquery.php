        ...
        if ($value['userent']) {
            ...
            if(isset($product_id)){
                ...
                if($exist > 0){
                    //Update here
                    $pdo = ORM::get_db();
                    $query = "UPDATE `".$config['db']['pre']."custom_data` set field_type = '".$field_type."', field_data = '".escape_html($field_data)."' where product_id = '".$product_id."' and field_id = '".$field_id."' LIMIT 1";
                    $pdo->query($query);
                    ...
                    if($field_data != "") {
                        ...
                        $field_insert->field_data = escape_html($field_data);
                        $field_insert->save();
                    }
                }
            }
    ...
    foreach ($result as $info)
    {
        ...
        if($info['custom_type'] == 'radio-buttons')
        {
            ...
            foreach($options as $key3=>$value3)
            {
                ...
                if($config['tpl_name'] == 'classic-theme'){
...
$where ORDER BY $order_by $pagelimit";
    ...
    if ($result) {
        foreach ($result as $info)
        {
            ...
            $item[$info['id']]['product_name'] = escape_html($info['product_name']);
            //$item[$info['id']]['desc'] = strlimiter(stripslashes(nl2br($info['description'])),25);
            $item[$info['id']]['desc'] = strlimiter($info['description'],110);
            $item[$info['id']]['cat_id'] = escape_html($info['category']);
            $item[$info['id']]['sub_cat_id'] = escape_html($info['sub_category']);
            $item[$info['id']]['price'] = escape_html($info['price']);
            ...
            $cityname = get_cityName_by_id(escape_html($info['city']));
            ...
            $item[$info['id']]['favorite'] = check_product_favorite(escape_html($info['id']));
            ...
            $item[$info['id']]['username'] = escape_html($userinfo['username']);
    ...
    if($payment_type == "subscr"){
        ...
        if(!empty($subsc_details)){
            ...
            elseif($txn_type == 'subscr_signup')
            {
                ...
                $upgrades_insert->sub_id = escape_html($subcription_id);
                $upgrades_insert->user_id = escape_html($user_id);
                $upgrades_insert->upgrade_lasttime = $now;
                $upgrades_insert->upgrade_expires = escape_html($expires);
                $upgrades_insert->unique_id = escape_html($unique_subscription_id);
                $upgrades_insert->stripe_customer_id = escape_html($subscription_stripe_customer_id);
                $upgrades_insert->stripe_subscription_id = escape_html($subscription_stripe_subscription_id);
                $upgrades_insert->billing_day = escape_html($subscription_billing_day);
                $upgrades_insert->length = escape_html($subscription_length);
                $upgrades_insert->interval = escape_html($subscription_interval);
                $upgrades_insert->trial_days = escape_html($subscription_trial_days);
                $upgrades_insert->status = escape_html($subscription_status);
                $upgrades_insert->date_trial_ends = escape_html($subscription_date_trial_ends);
            ...
            $trans_insert->product_name = escape_html($title);
